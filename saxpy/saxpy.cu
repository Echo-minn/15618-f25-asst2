...
saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    // following the kernel call to wait for completion of all CUDA work on the GPU
    ...
    printf("Kernel: %.3f ms\t\t[%.3f GB/s]\t\t%.3f %\n", 1000.f * kernelDuration, toBW(totalBytes, kernelDuration), 100.f * kernelDuration / overallDuration);
